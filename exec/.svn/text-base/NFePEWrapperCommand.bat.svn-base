...
rem Optimized for use with version 3.5.9-st of the Wrapper.
...
if not exist "%_WRAPPER_EXE%" (
    set _WRAPPER_EXE=%_REALPATH%%_WRAPPER_BASE%.exe
) else if not exist "%_WRAPPER_EXE%" (
...
if [%_PASS_THROUGH%]==[] ( 
    echo Usage: %0 [ console : start : pause : resume : stop : restart : install : update : remove : query ]
) else (
    echo Usage: %0 [ console {JavaAppArgs} : start : pause : resume : stop : restart : install {JavaAppArgs} : update {JavaAppArgs} : remove : query ]
)
pause
goto :eof
